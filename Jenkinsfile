    ...
    stages {
        stage('Test'){
            steps {
                ...
                sh 'docker run --rm -e SHELL=/bin/sh -e TYPO3_PATH_WEB -w /opt/typo3/web/typo3conf/ext/sh_scoutnet_webservice -v `pwd`:/opt/typo3/web/typo3conf/ext/sh_scoutnet_webservice -i scoutnet/cihost:latest find . -name \\*.php | parallel --gnu php -d display_errors=stderr -l {}'
            }
        }
        stage('Build'){
            steps {
